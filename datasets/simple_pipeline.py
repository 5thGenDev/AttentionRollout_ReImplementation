...
def load_dataset(dataset_name):
    match dataset_name:
        case "CIFAR10":
            data = datasets.CIFAR10("data",transform=build_transform(True,224),download=True, )
            train_dl = torch.utils.data.DataLoader(data,64,shuffle=True,num_workers=8,pin_memory=True)
            
            data = datasets.CIFAR10("data",False,transform=build_transform(False,224),download=True)
        ...
        case "OxfordIIITPet":
            data = datasets.OxfordIIITPet("data",transform=build_transform(True,224),download=True, )
            train_dl = torch.utils.data.DataLoader(data,64,shuffle=True,num_workers=8,pin_memory=True)
            
            data = datasets.OxfordIIITPet("data",False,transform=build_transform(False,224),download=True)
            val_dl = torch.utils.data.DataLoader(data,64,shuffle=False,num_workers=4)
